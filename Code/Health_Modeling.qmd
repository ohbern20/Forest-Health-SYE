...
```{r}
#first, I will rename Kip in Tree_Div 
Tree_Div <- Tree_Div |>
  mutate(Forest = as.character(Forest)) |>
  mutate(Forest = replace(Forest, Forest == 'Kip tract', 'Kip'))


for_modeling <- merge(merged_dens, Tree_Div, by = "Forest")
```
...
```{r}
mod1 <- lm(meanShannon ~ Mean_canopy_dens + Mean_ground_cover + Mean_regen_density, data = for_modeling) 

summary(mod1)
```

```{r}
mod2 <- lm(meanShannon ~ Mean_canopy_dens + Mean_ground_cover, data = for_modeling) 

summary(mod2)
```

```{r}
mod3 <- lm(meanShannon ~ Mean_canopy_dens + Mean_regen_density, data = for_modeling) 
...
```{r}
mod4 <- lm(meanShannon ~ Mean_ground_cover + Mean_regen_density, data = for_modeling) 
